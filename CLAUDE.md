...
- **`meta-gewgaw/` recipes** are all pure-data (no compile/configure) installs of
  ...
  - `gewgaw-collector` (`recipes-apps/`) → two Python daemons that record nearby 2.4 GHz Wi-Fi APs + BLE devices into a local SQLite presence DB (`gewgaw-collector`, long-running) and opportunistically upload new observations to AWS IoT Core over known/open Wi-Fi (`gewgaw-submit`, hourly timer + boot beacon), coordinating the single radio via an arbiter. Config in `/etc/gewgaw/`; schema in `/usr/share/gewgaw/schema.sql`; DB at `/var/lib/gewgaw/`; uploads via `aws-iot-mqtt pub`. Design + decisions: `scan_and_submit_daemons_design.md`. **Status: `gewgaw-collector` does real Wi-Fi (`iw`) + BLE (`bluetoothctl`) scanning into the SQLite presence model (phase 2) and hosts the single-radio arbiter — a unix-socket server at `GEWGAW_SOCK` (`/run/gewgaw/arbiter.sock`) speaking `ACQUIRE <secs>`/`RELEASE`/`PING`, with a connection-bound lease watchdog that auto-resumes Wi-Fi scanning if the holder times out, releases, or drops (phase 3). `gewgaw-submit` does the real steady-state `upload` run (phase 4): it counts closed/unsynced sessions, selects candidate APs visible now (known-PSK from `networks.conf` first, then discovered open if `ALLOW_OPEN=1`), `ACQUIRE`s the radio lease, writes a transient single-network `wpa_supplicant-wlan0.conf` per attempt, waits for association + DHCP, runs a busybox-`wget` 204 connectivity check, and batch-publishes sightings to `gewgaw/<thing>/sightings` via `aws-iot-mqtt pub` (marking rows `synced=1` only after each pub exits 0), then tears down and `RELEASE`s. `gewgaw-submit boot` is the real boot beacon (phase 5, `gewgaw-boot.service`, once per boot): it records a `boot` event into the `events` table *first* (decoupled delivery — a boot with no uplink is still reported later with its real time), waits up to `WAIT_FOR_SCAN` for the collector's first scan, connects with a larger lease (`BOOT_LEASE_SECONDS`) and a forgiving `BOOT_CONNECT_BUDGET` retry budget, confirms `NTPSynchronized` (`NTP_WAIT`) and re-stamps the boot event with the corrected clock + ip/ssid, then flushes events to `gewgaw/<thing>/status` followed by sightings. Both modes now flush events before sightings on every successful connect (§10). Candidate APs are filtered/scored by the per-BSSID `net_health` blacklist (phase 6, §9): each attempt is BSSID-pinned in the transient `wpa_supplicant` conf and, on assoc/dhcp/captive/no_internet failure, the BSSID gets an exponential-backoff `cooldown_until` (open: 15min→1h→6h→24h→7d, captive starts a rung higher; configured PSK nets get a gentle 5min→1h ladder); a success clears the streak. BSSIDs in cooldown are dropped from `select_candidates` and skipped mid-run. The remaining phases are the timer soak test and clock/GPS hardening.** Known-network PSKs live in the runtime-only `/etc/gewgaw/networks.conf` (wpa_supplicant-native, gitignored, never baked in — only `networks.conf.sample` is tracked).
