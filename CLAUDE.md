...
- **`meta-gewgaw/` recipes** are all pure-data (no compile/configure) installs of
  ...
  - `gewgaw-collector` (`recipes-apps/`) → two Python daemons that record nearby 2.4 GHz Wi-Fi APs + BLE devices into a local SQLite presence DB (`gewgaw-collector`, long-running) and opportunistically upload new observations to AWS IoT Core over known/open Wi-Fi (`gewgaw-submit`, hourly timer + boot beacon), coordinating the single radio via an arbiter. Config in `/etc/gewgaw/`; schema in `/usr/share/gewgaw/schema.sql`; DB at `/var/lib/gewgaw/`; uploads via `aws-iot-mqtt pub`. Design + decisions: `scan_and_submit_daemons_design.md`. **Currently phase-1 stubs** (schema/config/units final; scan + upload logic still to come). Known-network PSKs live in the runtime-only `/etc/gewgaw/networks.conf` (wpa_supplicant-native, gitignored, never baked in — only `networks.conf.sample` is tracked).
