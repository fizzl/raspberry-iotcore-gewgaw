...
- **`meta-gewgaw/` recipes** are all pure-data (no compile/configure) installs of
  ...
  - `gewgaw-collector` (`recipes-apps/`) → two Python daemons that record nearby 2.4 GHz Wi-Fi APs + BLE devices into a local SQLite presence DB (`gewgaw-collector`, long-running) and opportunistically upload new observations to AWS IoT Core over known/open Wi-Fi (`gewgaw-submit`, hourly timer + boot beacon), coordinating the single radio via an arbiter. Config in `/etc/gewgaw/`; schema in `/usr/share/gewgaw/schema.sql`; DB at `/var/lib/gewgaw/`; uploads via `aws-iot-mqtt pub`. Design + decisions: `scan_and_submit_daemons_design.md`. **Status: `gewgaw-collector` does real Wi-Fi (`iw`) + BLE (`bluetoothctl`) scanning into the SQLite presence model (phase 2); the radio arbiter and the `gewgaw-submit` upload path are later phases — `gewgaw-submit` is still a stub.** Known-network PSKs live in the runtime-only `/etc/gewgaw/networks.conf` (wpa_supplicant-native, gitignored, never baked in — only `networks.conf.sample` is tracked).
